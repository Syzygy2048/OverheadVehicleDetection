...
from tensorflow.contrib.slim.nets import resnet_v2 as nn_architecture
...
NUM_CLASSES = 10
CHECKPOINT_PATH = "checkpoints/resnet_v2_152/resnet_v2_152.ckpt"
BATCH_SIZE = 25
...
with slim.arg_scope(nn_architecture.resnet_arg_scope()):  # https://kwotsin.github.io/tech/2017/02/11/transfer-learning.html
    logits, endpoints = nn_architecture.resnet_v2_152(inputs=images,
                                                      num_classes=10)



retrain = ['resnet_v2_152/logits', 'resnet_v2_152/block4']  # could also remove some of the input channels (probably Conv2d_1a_3x3) to get rid of 3 color channel inputs and accept monochrome
...
variables_to_train = tf.trainable_variables('resnet_v2_152/logits') + tf.trainable_variables('resnet_v2_152/block4')
...
for i in range(5000):
    ...
    lbls = np.reshape(batch[1], (25, 1, 1, 10))
    _, summary = sess.run([train_step, summary_op], feed_dict={images: img, labels: lbls})

    summary_writer.add_summary(summary)

    with sess.as_default():
            train_accuracy = predictions  .eval(feed_dict={images: img, labels: lbls})
